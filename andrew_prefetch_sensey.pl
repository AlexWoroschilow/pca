...
sub do_align ($ $ $ $ $ $) {
  ...
  if ( $function == 2 || $function == 0 ) {
    $matrix_strct = score_mat_new( seq_size($seq_ptr1), seq_size($seq_ptr2) );
    score_pvec( $matrix_strct, $pvec1_strct, $pvec2_strct );
  }

  #   shift and score matrix
  my $smallsize = (
      coord_size($coord1) > coord_size($coord2)
    ? coord_size($coord2)
    : coord_size($coord1)
  );
  my $shift = ( $s_factor * $smallsize ) + $s_const;
  if ( $function == 0 ) {
    $tot_matrix = score_mat_add( $matrix_ca, $matrix_strct, $weight, $shift );
  }
  elsif ( $function == 1 ) {
...
sub read_struct_pl ($ $) {
  ...
  if ( $function == 1 || $function == 0 ) {
    ...
    if ( -e $vecfile_ca ) {
      ...
      my $ca_classfcn =
        ac_read_calpha( $CA_CLASSFILE, $tau_error, $ca_dist_error, $corr_num );
  ...
  if ( $function == 2 || $function == 0 ) {
    ...
    if ( -e $vecfile_strct ) {
      ...
      my $strct_classfcn = aa_strct_clssfcn_read( $CLASSFILE, $gauss_err );
...
sub do_lib (\@ \@ $ $ $ $ $ $ $ $ $) {
  my (
    ...
    $title,      $rmsd_thresh, $minFracDME,   $maxItNum,
  ...
  if ( $mkdret != 1 ) {
    if ( -e $htmldir ) {
      ...
      print STDERR
        "makedir failure for \"htmldir\" in  $RESDIR. Must die. $!\n";
      return EXIT_FAILURE;
    }
  }
  $htmldir =~ s/\n$//;
  ...
  $$text_to_mail .=
    "SALAMI has finished searching for proteins similar to $vecname.\n";
  $$text_to_mail .=
    " Your results can be viewed in our interactive results browser at:\n";
  $$text_to_mail .= " $url\n";
...
";
  ...
  addvec( $veclist,
    prob_vec_read("$PVEC_STRCT_DIR/$$structlist[$indices[0]].vec") );
  ...
  for ( my $i = 1 ; $i < $todo ; $i++ ) {
    addvec( $veclist,
      prob_vec_read("$PVEC_STRCT_DIR/$$structlist[$indices[$i]].vec") );
    $seq = coord_get_seq(
      coord_read(
...
MINFRAGDME: {    #for the dme thresh
    for ( my $i = 0 ; $i < ( $todo + $shift ) ; $i++ ) {
      ...
      while ( ( $$a[1] > $rmsd_thresh ) && ( $n > $MAG_NUM_ASREST ) ) {
        ...
        $testbitset =
          pair_set_sel_delmaxdistance( $$a[10], $coord11, $pair_set,
          $testbitset );
...
", $$a[8], $$a[9], $$a[6] / seq_size( coord_get_seq($coord1) ), $sid, $$a[6],
      ...
      if ( $i < 1 ) {
        $conserv = printconserv( $conservs, get_pair_set_n($malignm),
          $malignm, $sp1, get_pair_set_n($pair_set), $j );
        $strc = printconserv( $seqconserv, get_pair_set_n($malignm),
          $malignm, $sp1, get_pair_set_n($pair_set), $j );
      }
      else {
        $conserv = printconserv(
          $conservs, get_pair_set_n($malignm),
          $malignm, $sp1, get_pair_set_n($pair_set),
          $j + 1
        );
        $strc = printconserv(
          $seqconserv, get_pair_set_n($malignm),
          $malignm, $sp1, get_pair_set_n($pair_set),
          $j + 1
        );
      }
